...
`›` marks keyboard selection on session and host rows; bright mint, bold, underlined text marks active cockpit sessions. Both appear independently while sidebar is focused. Unfocused sidebar hides pointer and keeps active session emphasized and visible.
