    ...
    while True:
        ...
        if key in (curses.KEY_BACKSPACE, 8, 127):
            if chars:
                chars.pop()
            continue
        if 32 <= key <= 126 and len(chars) < max(0, w - len(prompt) - 1):
...
def _entry_text(entry: Entry, selected: bool) -> str:
    icon = _icons()
    ...
    if entry.kind == "session":
        ...
        return f"{pointer} {icon[kind]} {entry.label}"
...
def _draw_entries(stdscr: curses.window, entries: list[Entry], selected: int, h: int, w: int) -> None:
    ...
    for idx in range(start, end):
        if row >= h - 1:
            break
        entry = entries[idx]
        stdscr.addnstr(row, 0, _entry_text(entry, idx == selected), w - 1, _entry_attr(entry, idx == selected))
        row += 1
...
def _draw(stdscr: curses.window, entries: list[Entry], selected: int, status: str, filter_text: str, filtering: bool = False) -> None:
    stdscr.erase()
    h, w = stdscr.getmaxyx()
    _draw_title(stdscr, w, filter_text)
    _draw_entries(stdscr, entries, selected, h, w)
...
def run(stdscr: curses.window) -> None:
    ...
    while True:
        ...
        _draw(stdscr, entries, selected, status, filter_text, filtering)
        key = stdscr.getch()
        if filtering:
            if key in (27, 3):
                filter_text = ""
